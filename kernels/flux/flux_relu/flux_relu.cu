...
struct matmul_template {
    ...
    struct consumer {
        ...
        __device__ static void finish(consumer_finish_args<layout> args) {
            // one thing we need to know is how we can access to the data stored on the regs for each thread
            // although I don't know the detail, the rule is
            // 1. suppose the accum is rt_bf<16, 256>
            // 2. tk splits the rt_bf with base tile <16, 16>, resulting in a (1, 16) rt composed of base tile
            // 3. each thread is responsible for 16 * 256 / 32 = 128 data
            // 4. the 128 data is distributed to the 16 base tiles, meaning each thread is responsible for storing 8 elements from each base tile
            // 5. the 8 element is stored in pairs, i.e, 2 elements defined by base_tile_row, base_tile_col, and data j
            // 6. the question why each thread can use same pattern for accessing the data without worrying about the index might be due to
            //    the implementation of tk, where the accessing and index mapping is decoupled.
            // one thing to note is, let's say rt_bf is <x, y>, then, each thread needs to store xy / 32 elements. We will have x/16 * y/16 = xy/256
            // base tiles -> the number of data within a base tile that a thread needs to store is fixed at 8
            #pragma unroll
            for (int i = 0; i < args.state.accum.width; i++) {
                #pragma unroll
                for (int j = 0; j < 4; j++) {
                    args.state.accum.tiles[0][i].data[j].x = (args.state.accum.tiles[0][i].data[j].x > 0) ? args.state.accum.tiles[0][i].data[j].x : 0.0f;
                    args.state.accum.tiles[0][i].data[j].y = (args.state.accum.tiles[0][i].data[j].y > 0) ? args.state.accum.tiles[0][i].data[j].y : 0.0f;
                }
            }
...
void cpu_flux_relu(float* a, float* b, float* c, int M, int N, int K) {
...
template<typename mmt>
int run_benchmark(size_t M, size_t N, size_t K, bool check) {
    ...
    if(check) cpu_flux_relu(h_A, h_B, h_C_ref, M, N, K);
...
int main() {
    ...
    int N;
    N = 4096;
    run_benchmark<matmul_template<2,4,8>>(N, N, N, false);
    // N = 3072;
