    ...
    struct consumer {
        ...
        __device__ static void finish(consumer_finish_args<layout> args) {
            ...
            for (int i = 0; i < args.state.accum.width; i++) {
                #pragma unroll
                for (int j = 0; j < 4; j++) {
                    args.state.accum.tiles[0][i].data[j].x = 1.0f / (1 + expf(-args.state.accum.tiles[0][i].data[j].x));
                    args.state.accum.tiles[0][i].data[j].y = 1.0f / (1 + expf(-args.state.accum.tiles[0][i].data[j].y));
                }
            }
