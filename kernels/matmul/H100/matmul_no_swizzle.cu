...
struct matmul_template {
    ...
    __device__ static inline void common_setup(common_setup_args<layout> args) {
        // we don't need more than 1 outer iterations since each block only do one tile of output. So
        // if task_iter is larger than 1, simply set args.num_iters to be -1 and lcf will know we shall stop the entire iteration
        if (args.task_iter >= 1) {
            args.num_iters = -1;
        } else {
            // the inner dim is 64
        ...
        // we need to determine the sub_tile level row and col of this kernel
        int row = blockIdx.y * M_BLOCK;
        int col = blockIdx.x * N_BLOCK;
        // it is our job to determine
        // which block tile to work on, the starting row and col (args.common.coord) for each wgs (that is, in lcf, if it finds the wg as a consumer,
        // then, it will call finish based on what you have set. If there is wg partitioning, meaning, wg is responsible for different sub tiles of the block,
        // then we need to set up the corresponding row and col in common setup).
        int id = warpgroup::groupid() == NUM_CONSUMER_WARPS / 4 ? 0 : warpgroup::groupid(); // consumer has its own group id, but producer is set to 0
        // two consumer wgs have different starting position
        args.common.coord = { row + id, col };
    }
    struct producer {
...
int run_benchmark(size_t M, size_t N, size_t K) {
    ...
    // Launch kernel
    dim3 grid_size(mmt::template grid<false>(M, N, K));
    dim3 block(kittens::prototype::detail::NUM_THREADS_v<mmt>);
    std::cout << "Launching warmup kernel with grid (" << grid_size.x << ", " << grid_size.y << "), block (" << block.x << ")\n";
    for(int i = 0; i < (NCU ? 0 : 2); i++) { // warmup
        inner_run<mmt>(d_A, d_B, d_C, M, N, K, grid_size, block);
    }

    // Start timing
    cudaDeviceSynchronize();
    std::cout << "Launching kernel with grid (" << grid_size.x << ", " << grid_size.y << "), block (" << block.x << ")\n";
...
int main() {
    ...
    int N_list[] = {128, 256, 512, 1024, 2048, 4096, 8192, 16384};
    int len = 8;
    for (int i = 0; i < len; i++) {
        run_benchmark<matmul_template<2,4,8>>(N_list[i], N_list[i], N_list[i]);
    }
    // N = 3072;
